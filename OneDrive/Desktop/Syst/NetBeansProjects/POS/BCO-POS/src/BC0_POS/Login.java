...
public class Login extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        panel_login.setBackground(new java.awt.Color(14, 55, 89));
        ...
        jLabel8.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Icons/userrr.png"))); // NOI18N
        ...
        panel_create.setBackground(new java.awt.Color(14, 18, 14));

        jComboBox1.setBackground(new java.awt.Color(14, 18, 14));
        ...
        jPanel2.add(jLabel7, new org.netbeans.lib.awtextra.AbsoluteConstraints(410, 190, -1, -1));
...
try {
    if (username.getText().isEmpty() && password.getText().isEmpty()) {
        ...
        try (Statement state = Home_source.mycon().createStatement()) {
            String query = "SELECT username, type FROM tbllog WHERE username = ? AND password = ?";
            try (PreparedStatement preparedStatement = Home_source.mycon().prepareStatement(query)) {
                ...
                if (rs.next()) {
                    ...
                    if (userType == 1) {
                        // Admin login logic (your existing logic for admins)
                        Main_menu m = new Main_menu();
                        m.show();
                        m.log.setText(rs.getString("username"));
                        m.jComboBox2.setSelectedItem("Regular Customer");
                        m.jComboBox4.setSelectedItem("All");
                        m.jComboBox5.setSelectedItem("Product");

                        OOP.scave(m.POS, m.home, m.Costumer, m.Selling, m.Product, m.Stock_in, m.Income, m.Removed, m.Invoice);
        m.jComboBox2.setSelectedItem("Regular Costumer");
        

                        dispose();
                    } else if (userType == 2) {
                        // Non-admin login logic (display POS.java, for example)
                        POS pos = new POS();
                        pos.setVisible(true);
                        dispose();
                        OOP.scave(m.POS, m.home, m.Costumer, m.Selling, m.Product, m.Stock_in, m.Income, m.Removed, m.Invoice);
        p.jComboBox2.setSelectedItem("Regular Costumer");
        p.itemField1.setText("Input Bar Code: ");
                    }
                } else {
...
if (selectedItem != null) {
    ...
    // Assuming you have specific values for user and admin in the combo box
    if ("User".equals(selectedValue)) {
        type = 2;
    } else if ("Admin".equals(selectedValue)) {
        type = 1;
    } else {
        // Handle other cases or default value
        type = 0; // Default to 0 or another suitable value
    }
} else {
...
try {
    if (user.equals("Enter username") || pass.equals("Enter password") || type == 0) {
        JOptionPane.showMessageDialog(rootPane, "Please input data..");
    } else {
        try (Statement state = Home_source.mycon().createStatement()) {
            state.execute("INSERT INTO tbllog (`username`, `password`, `type`) VALUES ('" + user + "','" + pass + "','" + type + "')");
            panel_login.setVisible(true);
            panel_create.setVisible(false);
            JOptionPane.showMessageDialog(rootPane, "Added Successfully", String.valueOf(type), JOptionPane.INFORMATION_MESSAGE);
        }
    }
} catch (Exception x) {
    ...
    public static javax.swing.JPanel panel_create;
